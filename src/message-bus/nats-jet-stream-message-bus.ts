...
export class NatsJetStreamMessageBus implements IMessageBus {
  constructor(
    private readonly channel: NatsJetStreamChannel,
  ) {
  }
  ...
  async dispatch(message: RoutingMessage): Promise<object | void> {
    ...
    let routingKey = this.channel.config.consumerSubject;
    const h = headers();

    if (this.channel.config.consumerSubject.includes('>') || this.channel.config.consumerSubject.includes('*')) {
      routingKey = message.messageRoutingKey;
    }

    h.set('messaging-routing-key', message.messageRoutingKey);

    js.publish(routingKey, JSON.stringify(message.message), { headers: h });
  }
}
